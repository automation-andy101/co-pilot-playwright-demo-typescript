...
## Run Tests
# With Tags
npm run test --TAGS="@fail"

# With Tags and browser - firefox
npm run test --TAGS="@fail" --BROWSER="firefox"

# Run feature files using the Before hook that does not use the @auth tag
npm run test --TAGS="not @auth"

# Generate a credentials file 
npx playwright codegen https://bookcart.azurewebsites.net/login --save-storage=admin.json